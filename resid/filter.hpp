...
namespace Plus4 {
  ...
  public:
    ...
    PLUS4EMU_INLINE void clock(int voice1, int voice2, int voice3);
  ...
  protected:
    ...
    PLUS4EMU_INLINE int solve_gain(int* opamp, int n, int vi_t, int& x,
                                   model_filter_t& mf);
    PLUS4EMU_INLINE int solve_integrate_6581(int dt, int vi_t, int& x, int& vc,
                                             model_filter_t& mf);
  ...
  // --------------------------------------------------------------------------
  PLUS4EMU_INLINE void Filter::clock(int voice1, int voice2, int voice3)
  {
  ...
  // --------------------------------------------------------------------------
  PLUS4EMU_INLINE void Filter::clock(cycle_count delta_t,
                                     int voice1, int voice2, int voice3)
  {
    model_filter_t& f = model_filter[sid_model];
  ...
  // SID audio input (16 bits).
  // --------------------------------------------------------------------------
  PLUS4EMU_INLINE void Filter::input(short sample)
  {
  ...
  */
  PLUS4EMU_INLINE int Filter::solve_gain(int* opamp, int n, int vi, int& x,
                                         model_filter_t& mf)
  {
    ...
    for (;;) {
      ...
      if (PLUS4EMU_UNLIKELY(x <= ak) || PLUS4EMU_UNLIKELY(x >= bk)) {
        // Bisection step (ala Dekker's method).
        x = (ak + bk) >> 1;
        if (PLUS4EMU_UNLIKELY(x == ak)) {
  ...
  */
  PLUS4EMU_INLINE int Filter::solve_integrate_6581(int dt, int vi,
                                                   int& vx, int& vc,
                                                   model_filter_t& mf)
  {
    // Note that all variables are translated and scaled in order to fit
