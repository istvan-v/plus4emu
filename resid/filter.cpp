...
namespace Plus4 {
  ...
  static inline double log_1p(double x)
  {
  ...
  Filter::Filter()
  {
    ...
    if (!class_init) {
      ...
      for (int m = 0; m < 2; m++) {
        ...
        // Normalized snake current factor, 1 clock cycle.
        // Fit in 5 bits.
        mf.n_snake = (int)(denorm*(1 << 13)
                           *(fi.uCox/(2*fi.k)
                             *fi.WL_snake*(1.0/RESID_CLOCK_FREQUENCY)/fi.C)
                           + 0.5);
        ...
        for (int i = 0; i < fi.opamp_voltage_size; i++) {
          ...
          //
          scaled_voltage[fi.opamp_voltage_size - 1 - i][0] =
              int((N16*(fi.opamp_voltage[i][1] - fi.opamp_voltage[i][0])
                   + (1 << 16))/2 + 0.5);
          scaled_voltage[fi.opamp_voltage_size - 1 - i][1] =
        ...
        mf.vc_max = (int)(N30*(fi.opamp_voltage[0][1]
                               - fi.opamp_voltage[0][0]));
        mf.vc_min =
            (int)(N30*(fi.opamp_voltage[fi.opamp_voltage_size - 1][1]
                       - fi.opamp_voltage[fi.opamp_voltage_size - 1][0]));
        ...
        for (int n = 0; n < (1 << bits); n++) {
          mf.f0_dac[n] =
              (unsigned short)(N16*(fi.dac_zero
                                    + mf.f0_dac[n]*fi.dac_scale/(1 << bits)
                                    - vmin) + 0.5);
        }
      }
      ...
      */
      ...
      // Normalized current factor for 1 clock cycle.
      double N15 = N16/2;
      double n_Is = N15*(1.0/RESID_CLOCK_FREQUENCY)/fi.C*Is;

      // kVg_Vx = k*Vg - Vx
      // I.e. if k != 1.0, Vg must be scaled accordingly.
      for (int kVg_Vx = 0; kVg_Vx < (1 << 16); kVg_Vx++) {
        double log_term = log_1p(exp((kVg_Vx/N16 - kVt)/(2*Ut)));
        // Scaled by m*2^15
  ...
  void Filter::set_w0()
  {
    ...
    w0 = 82355*(fc + 1) >> 11;
#else
    // adjust for the actual clock frequency
    w0 = int(82354966458.264/RESID_CLOCK_FREQUENCY + 0.5)*(fc + 1) >> 11;
#endif
  }
