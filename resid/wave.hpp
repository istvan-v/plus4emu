...
namespace Plus4 {
  ...
  public:
    ...
    PLUS4EMU_INLINE void clock(cycle_count delta_t);
    PLUS4EMU_INLINE void synchronize();
    void reset();
    ...
    PLUS4EMU_INLINE void set_waveform_output();
    PLUS4EMU_INLINE void set_waveform_output(cycle_count delta_t);

  protected:
    PLUS4EMU_INLINE void clock_shift_register();
    PLUS4EMU_INLINE void write_shift_register();
    PLUS4EMU_INLINE void reset_shift_register();
    PLUS4EMU_INLINE void set_noise_output();
  ...
  // --------------------------------------------------------------------------
  PLUS4EMU_INLINE void WaveformGenerator::clock()
  {
    if (PLUS4EMU_UNLIKELY(test)) {
      // Count down time to fully reset shift register.
      if (PLUS4EMU_UNLIKELY(shift_register_reset) &&
          PLUS4EMU_UNLIKELY(!--shift_register_reset)) {
      ...
      if (PLUS4EMU_UNLIKELY(accumulator_bits_set & 0x080000)) {
        // Pipeline: Detect rising bit, shift phase 1, shift phase 2.
        shift_pipeline = 2;
      }
      else if (PLUS4EMU_UNLIKELY(shift_pipeline) && !--shift_pipeline) {
  ...
  // --------------------------------------------------------------------------
  PLUS4EMU_INLINE void WaveformGenerator::clock(cycle_count delta_t)
  {
    if (PLUS4EMU_UNLIKELY(test)) {
      // Count down time to fully reset shift register.
      if (shift_register_reset) {
        shift_register_reset -= delta_t;
        if (PLUS4EMU_UNLIKELY(shift_register_reset <= 0)) {
          reset_shift_register();
        }
      }
  ...
  // --------------------------------------------------------------------------
  PLUS4EMU_INLINE void WaveformGenerator::synchronize()
  {
    // A special case occurs when a sync source is synced itself on the same
    // cycle as when its MSB is set high. In this case the destination will
    // not be synced. This has been verified by sampling OSC3.
    if (PLUS4EMU_UNLIKELY(msb_rising) && sync_dest->sync &&
  ...
  PLUS4EMU_INLINE void WaveformGenerator::clock_shift_register()
  {
  ...
  PLUS4EMU_INLINE void WaveformGenerator::write_shift_register()
  {
  ...
  PLUS4EMU_INLINE void WaveformGenerator::reset_shift_register()
  {
  ...
  PLUS4EMU_INLINE void WaveformGenerator::set_noise_output()
  {
    noise_output =
  ...
  PLUS4EMU_INLINE void WaveformGenerator::set_waveform_output()
  {
    // Set output value.
    if (PLUS4EMU_EXPECT(bool(waveform))) {
      ...
      if (PLUS4EMU_UNLIKELY(waveform > 0x8) &&
          PLUS4EMU_EXPECT(!test) && PLUS4EMU_EXPECT(shift_pipeline != 1)) {
      ...
      cycle_count delta_t)
  {
    ...
    if (PLUS4EMU_EXPECT(bool(waveform))) {
      ...
      osc3 = waveform_output;
      if (PLUS4EMU_UNLIKELY(waveform > 0x8) && PLUS4EMU_EXPECT(!test)) {
        // Combined waveforms write to the shift register.
      ...
      if (PLUS4EMU_EXPECT(bool(floating_output_ttl))) {
        ...
        if (PLUS4EMU_UNLIKELY(floating_output_ttl <= 0)) {
  ...
  PLUS4EMU_INLINE short WaveformGenerator::output()
  {
